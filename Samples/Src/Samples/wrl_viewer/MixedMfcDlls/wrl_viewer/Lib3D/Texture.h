...
public :
	...
	// File reading
	int ReadFile(TCHAR *filename,unsigned int width=-1,
		           unsigned int height=-1,unsigned int depth=-1);
	int ReadFileBMP(TCHAR *filename);
	int ReadFileRAW(TCHAR *filename,unsigned int width,
		              unsigned int height,unsigned int depth);
	
	// File saving
	int SaveFile(TCHAR *filename);
	int SaveFileBMP(TCHAR *filename);
	int SaveFileRAW(TCHAR *filename);
