...
int CMesh3d::WriteFile(CStdioFile &file)
{
	...
	{
		// Comment
		string.Format(_T("# Mesh : %d vertices, %d faces\n"),NbVertex(),NbFace());
		...
		for(int i=0;i<NbVertex;i++)
		{
			string.Format(_T("          %g %g %g"),m_ArrayVertex[i]->x(),
		                                     m_ArrayVertex[i]->y(),
																	       m_ArrayVertex[i]->z());
			file.WriteString(string);
			if(i!=(NbVertex-1))
		...
		for(i=0;i<NbFace;i++)
		{
			string.Format(_T("          %d, %d, %d, -1"),m_ArrayVertex.IndexFrom(m_ArrayFace[i]->v(0)),
	...
	CATCH(CFileException, e)
	{
		...
		#endif
		AfxMessageBox(_T("Error during writing transform"));
		return 0;
	}
	END_CATCH
...
int CMesh3d::WriteFileRaw(CFile &file)
{
	...
	CATCH(CFileException, e)
	{
		...
		#endif
		AfxMessageBox(_T("Error during writing"));
		return 0;
	}
	END_CATCH
